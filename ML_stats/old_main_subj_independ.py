...
def main():
    ...
    data_df = data_df[columns_to_select + ['ATCO']]
    ...
    if CHS:
        if BINARY:
            y_train = [1 if score < 4 else 2 for score in y_train]
        else:
            y_train = [1 if score < 2 else 3 if score > 3 else 2 for score in y_train]
    else:
        if  BINARY:
    ...
    if CHS:
        if BINARY:
            y_test = [1 if score < 4 else 2 for score in y_test]
        else:
            y_test = [1 if score < 2 else 3 if score > 3 else 2 for score in y_test]
    else:
        if  BINARY:
            th = getEEGThreshold(y_train)
